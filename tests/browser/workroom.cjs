...
(async()=>{
 ...
 if(!(await page.locator('#outcome-guide').innerText()).includes('separate checks for the selected scenario, not an overall success score')) throw Error('outcome flags presented without scenario-specific interpretation');
 await page.locator('#outcome-help > summary').click();
 const outcomeHelp = await page.locator('#outcome-help').innerText();
 if(!outcomeHelp.includes('yes while authorized work continues') || !outcomeHelp.includes('Refusal prose inside an action statement does not count') || !outcomeHelp.includes('check.initial') || !outcomeHelp.includes('check.latest')) throw Error('structured decline or repair-report check definitions are missing');
 await page.locator('#outcome-help > summary').click();
 ...
 const staticPlanSummary = await page.locator('#study-summary').textContent();
 if(!staticPlanSummary.includes('Planned schedule: 8 trials') || !staticPlanSummary.includes('execution counts appear in recorded progress below') || /Started|Provider completed|Evaluable/.test(staticPlanSummary)) throw Error('static schedule displays initial execution zeroes');
 ...
 if(process.env.PEB_TEST_STUDIES){
  ...
  if(await page.locator('#study-summary').textContent() !== staticPlanSummary) throw Error('execution altered the static schedule summary');
  const completedCounts = await page.locator('#study-execution-counts').textContent();
  if(!completedCounts.includes('Started 8') || !completedCounts.includes('Provider completed 8')) throw Error('recorded execution counters absent from the results block');
  ...
  if(!await page.locator('#study-execution-trials tr').nth(1).textContent().then(text => text.includes('No dispatch'))) throw Error('undispatched row hides the absence of a dispatched run');
  if(!(await page.locator('#study-execution-counts').textContent()).includes('Started 1') || await page.locator('#study-summary').textContent() !== staticPlanSummary) throw Error('partial live counters confused with the static plan');
